...
| **Redocking RMSD (top-ranked pose)** | **1.76 Å** — **LENIENT convention**: element-swap matching allows permutation of chemically equivalent atoms; a connectivity-constrained match gives 1.91 Å and name-based matching 2.52 Å. Formal graph-isomorphism/protonation-aware revalidation pending — treat as PASS-with-caveat |
...
| Ensemble re-docking of top 20 (5I81 / 5JG8) | 5I81 |Δ| median 0.17 (max 0.34) kcal/mol; 5JG8 systematically ~0.3–1.4 lower but all hits −6.99…−9.07; top-4 clean hits stable in all three crystals |
...
human SMPD1 = 49 µM, activity 375434). These chemotypes still demonstrate that the
protocol yields ≤ −7.0 scores for sphingomyelinase-family binders, but they are NOT a
human-SMPD1 potency anchor. The campaign's −7.0 threshold is retained only as an
...
- Receptor is a single static conformation (no ensemble docking performed in this
  campaign); flexible-loop (SMD/MD) effects on the entry channel are unassessed.
