...
pocket_ords = {id2ord[r] + 1 for r in CRYSTAL_POCKET}  # fixed pdb resid = ordinal+1
...
for fi in range(t.n_frames):
    box = t.unitcell_vectors[fi]
    L = np.array(box); Linv = np.linalg.inv(L)
    # whole-molecule imaging of ligand & pocket relative to frame-0 COM
    x = t.xyz[fi] * 10
    x0 = t.xyz[0] * 10
    def image_group(idx):
        ...
        return dcom - (frac @ L)   # signed correction vector (nm-space? careful)
    # NOTE: do imaging in A consistently
    ...
    R = Vt.T @ np.diag([1, 1, d]) @ U.T
    tt = qm_ - R @ pm_
    lig_in_prot = (xlig - xbb.mean(axis=0)) @ R.T + xbb.mean(axis=0) @ R.T + tt - tt + xlig*0  # placeholder
    # simpler: apply R,t directly
...
json.dump(out, open(f'{WORK}/runs/audit-20260913/tasks/sim-02/attempt-1/pocket_residence.json', 'w'), indent=1)
