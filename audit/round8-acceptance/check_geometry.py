...
exec(compile(source.read_text().split('occ = 0')[0],str(source),'exec'),ns)
ns['rows']=json.loads((out/'pocket_residence_r7_frames.json').read_text())
ns['summary']=json.loads((out/'pocket_residence_r7_summary.json').read_text())
